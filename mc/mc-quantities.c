...
void mc_initialize(log_gobbled *logg)
{
...
void mc_available(quantities_described qd,
                  log_gobbled *logg)
{
  int mod, low;
  char letter;
  ...
  for (i = 0; i < Max_quantities; i++)
  {
    ...
    if (letter && qd[i].available == 0 && (strchr("iIT", letter) == 0 || mod == -1))
    {
      ...
      {
        qd[i].available =
            low == -1 && (mod == -1 || letter == 'E' && (mod == 0 || mod == 1) || letter == 'K' && mod == 0 || letter == 'm' && mod <= 5
#if 0
                              || letter=='H' && mod==0
#endif
                          || letter == 'D' || letter == 'Q')
...
void mc_evaluate(quantities_described qd,
                 quantities_held *qh,
                 log_gobbled *logg)
{
  mc_iter *it;
  ...
  for (i = 0; i < Max_quantities; i++)
  {
    ...
    if (letter && !qh->updated[i] && (strchr("iIT", letter) == 0 || mod == -1))
    {
      ...
      case 'T':
      {
        if (it == 0 || logg->index['i'] != logg->last_index)
          break;
      ...
      case 'I':
      {
        ...
        sch = logg->data['m'];
        for (j = 0; j < Max_temps && sch->sched[j].inv_temp != 1 && sch->sched[j].inv_temp != inv_temp; j++)
          ;
        if (sch->sched[j].inv_temp != inv_temp)
          break;
      ...
      case 'j':
      {
        mc_temp_state *ts;
        ts = logg->data['b'];
        if (ts == 0 || logg->index['b'] != logg->last_index)
          break;
      ...
      case 'm':
      {
        ...
        case 3:
        {
          *qh->value[i] = it->move_point + it->spiral_offset - it->spiral_switch;
          break;
        }
        case 4:
        {
        ...
        {
          *qh->value[i] =
              abs(it->move_point + it->spiral_offset - it->spiral_switch) - abs(it->spiral_offset - it->spiral_switch);
          break;
        }
        default:
          abort();

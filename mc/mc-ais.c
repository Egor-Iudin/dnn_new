     ...
     char **argv)
{
  ...
  for (pass = 1; pass <= 2; pass++)
  {
    ...
    while (*ap)
    {
      ...
      while (!logf.at_end && (logf.header.index < lindex || logf.header.index % index_mod != 0))
      {
      ...
      for (;;)
      {
        ...
        if (ts != 0 || it != 0)
        {
          ...
          if (pass == 2)
          {
            ...
            ww_var[i] += (ww - 1) * (ww - 1);
            logw_var[i] += (it->log_weight - logw_mean[i]) * (it->log_weight - logw_mean[i]);
          }
        }

        /* Skip to next desired index, or to end of range. */

        while (!logf.at_end && logf.header.index <= hindex && logf.header.index % index_mod != 0)
        {
          log_file_forward(&logf);
        }

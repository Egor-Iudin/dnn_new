     ...
     char **argv)
{
  ...
  if (*ap == 0 || sscanf(*ap++, "%d%c", &n_traj, &junk) != 1 || n_traj < 1 || *ap == 0 || sscanf(*ap++, "%d%c", &min_steps, &junk) != 1 || min_steps < 0 || *ap == 0 || sscanf(*ap++, "%d%c", &max_steps, &junk) != 1 || max_steps < min_steps)
  {
    usage();
  }
  ...
  if (*ap == 0 || **ap == '/' && (sscanf(1 + *ap++, "%lf%c", &inv_temp, &junk) != 1 || inv_temp <= 0 || (inv_temp = 1 / inv_temp) < 1) || **ap != '/' && (sscanf(*ap++, "%lf%c", &inv_temp, &junk) != 1 || inv_temp <= 0 || inv_temp > 1) || *ap == 0 || sscanf(*ap++, "%lf%c", &decay, &junk) != 1 || decay <= 0 || decay > 1)
  {
    usage();
  }
    ...
    while (j != max_steps + 1)
    {
      ...
      if (j % modulus == 0 && (pre_save || j >= min_steps))
      {
        ...
        else
        {
          ...
          it->log_weight = -it->log_weight - 0.5 * ds.dim * log(inv_temp) + min_steps * ds.dim * log(decay) + log(max_steps - min_steps + 1.0);

...
void mc_record_sizes(log_gobbled *logg)
{
...
double mc_kinetic_energy(mc_dynamic_state *ds)
{
  double s;
  int j;
...
void mc_temp_present(mc_dynamic_state *ds, /* State to update */
...
int mc_temp_index(mc_temp_sched *sch, /* Schedule of inverse temperatures */
...
double mc_energy_diff(mc_dynamic_state *ds, /* Dynamical state structure */
                      mc_temp_sched *sch,   /* Tempering schedule */
...
)
{
  ...
  if (ds->temp_state == 0 || ds->temp_state->inv_temp == 1 && dir > 0 || ds->temp_index == 0 && dir < 0)
  {
    abort();
  }
...
void mc_value_copy(mc_value *dest, /* Place to copy to */

  ...
  for (ap = argv + argc - 1; ap > argv + 1; ap--)
  {
    ...
    else
    {
      for (i = n - 1; i >= 0; i--)
      {
        ...
        {
          sch->sched[t + n - 1 - i].inv_temp = exp(log(inv_temp) + (log(sch->sched[t - 1].inv_temp) - log(inv_temp)) * i / n);
        }
...
static void display_sched(log_gobbled *logg)
{
  mc_temp_sched *sch;
  int t;

...
static int temp_index(mc_temp_sched *sch, /* Schedule of inverse temperatures */
     ...
     char **argv)
{
  ...
  while (!logf_in.at_end)
  {
    ...
    if (logg.last_index < 0 || temp != 0 && index < 0 && (*temp == sch->sched[0].inv_temp || *temp == sch->sched[-index].inv_temp) || temp != 0 && index >= 0 && *temp == sch->sched[index].inv_temp)
    {
      for (i = 0; i < 128; i++)
      {

     ...
     char **argv)
{
  ...
  if (argc == 2 || argc == 3 && (*argv[2] >= '0' && *argv[2] <= '9' || strcmp(argv[2], "all") == 0))
  {
    index = argc == 2 ? -1 : strcmp(argv[2], "all") == 0 ? -2
                                                         : atoi(argv[2]);
    ...
    else if (index == -2)
    {
      while (!logf.at_end)
      {
        ...
        if (logg.data['o'] != 0 && logg.index['o'] == logg.last_index || logg.data['t'] != 0 && logg.index['t'] == logg.last_index)
        {
    ...
    else
    {
      ...
      if (logg.index['o'] != index)
        logg.data['o'] = 0;
      if (logg.index['t'] != index)
        logg.data['t'] = 0;
  ...
  while (*ap && strcmp(*ap, "/") != 0)
  {
    ...
    else if (strcmp(*ap, "heatbath") == 0)
    {
      ...
      {
        if ((ops->op[o].heatbath_decay = atof(*ap++)) < 0)
          usage();
      }
    }
    ...
    else if (strcmp(*ap, "mix-momentum") == 0)
    {
      ...
      if (!*ap || (ops->op[o].heatbath_decay = atof(*ap++)) < 0)
        usage();
    }
    ...
    else if (strcmp(*ap, "metropolis") == 0 || strcmp(*ap, "met") == 0 || strcmp(*ap, "rgrid-met") == 0)
    {
      ops->op[o].type =
      ...
      while (*ap && (*ap)[0] == '-' && strchr("abcdefghijklmnopqrstuvwxyz", (*ap)[1]))
      {
        if (strcmp("-b", *ap) == 0)
        {
          ops->op[o].b_accept = 1;
      ...
      if (*ap && strchr("0123456789+-.", **ap))
      {
        if ((ops->op[o].stepsize_adjust = atof(*ap)) == 0)
          usage();
        if (strchr(*ap, ':') != 0)
        {
          if ((ops->op[o].stepsize_alpha = atof(strchr(*ap, ':') + 1)) == 0)
            usage();
        }
        ap += 1;
      }
    ...
    else if (strcmp(*ap, "slice") == 0)
    {
      ...
      while (*ap && (*ap)[0] == '-' && strchr("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", (*ap)[1]))
      {
        if (strcmp("-g", *ap) == 0)
        {
          ops->op[o].g_shrink = 1;
    ...
    else if (strcmp(*ap, "met-1") == 0 || strcmp(*ap, "rgrid-met-1") == 0 || strcmp(*ap, "slice-1") == 0 || strcmp(*ap, "slice-over") == 0)
    {
      ops->op[o].type = strcmp(*ap, "met-1") == 0         ? 'm'
                        : strcmp(*ap, "rgrid-met-1") == 0 ? 'g'
                        : strcmp(*ap, "slice-1") == 0     ? 'S'
                                                          : 'O';
      ...
      while (*ap && (*ap)[0] == '-' && strchr("abcdefghijklmnopqrstuvwxyz", (*ap)[1]))
      {
        if (strcmp("-b", *ap) == 0 && (ops->op[o].type == 'm' || ops->op[o].type == 'g'))
        {
          ops->op[o].b_accept = 1;
        }
        ...
        else if (ops->op[o].type == 'S' && strcmp("-s", *ap) == 0)
        {
          ...
          {
            if (sscanf(*ap, "%d%c", &ops->op[o].s_factor, &junk) != 1 || ops->op[o].s_factor == 0)
            {
              usage();
            }
          }
      ...
      if (ops->op[o].type == 'O')
      {
        if (*ap && strchr("0123456789", **ap))
        {
          if ((ops->op[o].refinements = atoi(*ap)) < 0)
            usage();
          ap += 1;
        }
        ...
        if (*ap && strchr("0123456789.", **ap))
        {
          if ((ops->op[o].refresh_prob = atof(*ap)) < 0)
            usage();
          ap += 1;
        }
      }
    ...
    else if (strcmp(*ap, "dynamic") == 0 || strcmp(*ap, "permuted-dynamic") == 0 || strcmp(*ap, "slice-inside") == 0 || strcmp(*ap, "slice-outside") == 0
#if 0
    ...
    )
    {
      ...
      if (ops->op[o].type == 'o')
      {
        ...
        if (strchr(*ap, '/') != 0)
        {
          if ((ops->op[o].in_steps = atoi(strchr(*ap, '/') + 1)) <= 0 || ops->op[o].in_steps > ops->op[o].steps)
            usage();
        }
      }
    ...
    else if (strcmp(*ap, "hybrid") == 0 || strcmp(*ap, "tempered-hybrid") == 0 || strcmp(*ap, "spiral") == 0 || strcmp(*ap, "double-spiral") == 0)
    {
      ...
      else
      {
        ...
        if ((ops->op[o].temper_factor = atof(*ap)) <= 0)
          usage();
      }
      ...
      {
        s = strchr(*ap, '/') + 1;
        if ((ops->op[o].in_steps = atoi(s)) <= 0)
          usage();
        if (strchr(s, ':') != 0)
        {
          if ((ops->op[o].jump = atoi(strchr(s, ':') + 1)) <= 0)
            usage();
    ...
    else if (strcmp(*ap, "repeat") == 0)
    {
      ...
      if ((ops->op[o].repeat_count = atoi(*ap++)) <= 0)
        usage();
...
static void display_specs(log_gobbled *logg)
{
  mc_ops *ops;
  mc_traj *traj;
  int depth;
  ...
  if (logg->data['o'] != 0)
  {
    ...
    for (o = 0; o < Max_mc_ops && ops->op[o].type != 0; o++)
    {
      ...
      case 'S':
      {
        ...
        else if (ops->op[o].stepsize_adjust != 1 || ops->op[o].steps != 0 || ops->op[o].firsti != -1)
        {
          printf(" %.4f", ops->op[o].stepsize_adjust);
        }
      ...
      case 'O':
      {
        ...
        if (ops->op[o].refinements != 0 || ops->op[o].refresh_prob != 0 || ops->op[o].stepsize_alpha != 0 || ops->op[o].stepsize_adjust != 1 || ops->op[o].firsti != -1)
        {
          printf(" %d", ops->op[o].refinements);
        }
        if (ops->op[o].refresh_prob != 0 || ops->op[o].stepsize_alpha != 0 || ops->op[o].stepsize_adjust != 1 || ops->op[o].firsti != -1)
        {
      ...
      case 'A':
      {
        ...
        if (ops->op[o].app_param != 0)
          printf(" %.4f", ops->op[o].app_param);
        if (ops->op[o].app_param2 != 0)
          printf(" %.4f", ops->op[o].app_param2);
        printf("\n");
        break;
      }

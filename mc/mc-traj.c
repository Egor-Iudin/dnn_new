...
void mc_traj_init(mc_traj *tj0, /* Description of how to compute trajectory */
...
void mc_trajectory(mc_dynamic_state *ds, /* Dynamical state to update */
...
)
{
  ...
  case 'G':
  {
    ...
    if (tj->halfp)
    {
      ...
      // printf("MC_traj Iterations count = %d",iterationsCount);
    }
    else
    {
  ...
  case 'L':
  {
    if (tj->halfp)
    {
      ...
      {
        printf("mc_app_energy 18 TTT\n");
        mc_app_energy(ds, na, it->approx_order[0], 0, ds->grad);
      }

      for (k = 0; k < ds->dim; k++)
      {
        // printf("mc_app_energy 118 TTT\n");
        ds->p[k] -= sfh * ds->stepsize[k] * ds->grad[k];
      }
      a = 0;
      fprintf(stderr, "stepsize =  %f grad = %f \n",
              ds->stepsize[20], ds->grad[20]);
      ...
      for (;;)
      {
        ...
        for (k = 0; k < ds->dim; k++)
        {
          // printf("mc_app_energy 119 TTT\n");

          ds->q[k] += sf * ds->stepsize[k] * ds->p[k];

          //	fprintf(stderr, "stepsize =  %f grad = %f ",
          //		ds->stepsize[k], ds->grad[k] );
        }
        // fprintf(stderr,"\n");
        fprintf(stderr, "stepsize =  %f grad = %f \n",
                ds->stepsize[10], ds->grad[10]);

        a = (a + x + ta) % ta;
        o = a < na ? a : a == na ? 0
                                 : 2 * na - a;
        printf("mc_app_energy 19 TTT\n");

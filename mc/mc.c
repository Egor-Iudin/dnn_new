     ...
     char **argv)
{
  ...
  if ((max = atoi(**ap == '@' ? *ap + 1 : *ap)) <= 0)
    usage();
  ap += 1;

  if (*ap != 0 && strchr("0123456789", **ap) != 0 && ((modulus = atoi(*ap++)) <= 0))
    usage();
  ...
  {
    ap += 1;
    if (*ap == 0 || (decay = atof(*ap)) <= 0 && strcmp(*ap, "0") != 0)
      usage();
    ap += 1;
    if (*ap != 0)
    {
      if ((temperature = atof(*ap)) <= 0 && strcmp(*ap, "0") != 0)
        usage();
      ap += 1;
    }
  }
  ...
  {
    printf("first iter4");
    if (quantities[0] == 't' && quantities[1] == 't' && quantities[2] != 0 && strchr("12", quantities[2]) != 0 && quantities[3] == 0)
    {
  ...
  char *FileType = ".log";
  strcat(netFileName, FileType);
  while (!coalesced &&
         ((!timelimit && index <= max) || (timelimit && it->time < 60000 * max)))
  {
    /* Do the work. */
    // getchar();
    printf("mc_iteration started!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
    IterationNumber++;
    printf("mc iteration Number %d XXXXXXXXXXXXXXXXXXXXx\n", IterationNumber);
    char buff[100];
    time_t now = time(0);
    strftime(buff, 100, "%Y%m%d", localtime(&now));
    printf("mc iteration Start %s\n", buff);
    // mc_app_initialize(&logg,&ds);
    // mc_iter_init(&ds,ops,tj,sch);
    FILE *fp;

    ds.netFileName = netFileName;
    fp = fopen(ds.netFileName, "a+");
    fprintf(fp, "Start trainning   %s numder %d \n", argv[1], IterationNumber);
    fclose(fp);
    // getchar();
    mc_iteration(&ds, it, &logg, qd, N_quantities, IterationNumber, argv[1]);

    now = time(0);
    strftime(buff, 100, "%Y-%m-%d %H:%M:%S.000", localtime(&now));
    printf("mc iteration Start stop %s\n", buff);
    ...
    if (index % modulus == 0)
    {
      ...
      if (ds.temp_state != 0)
      {
        printf("ds.temp_state %d\n", sizeof(ds.temp_state));
        logf.header.type = 'b';
        logf.header.index = index;

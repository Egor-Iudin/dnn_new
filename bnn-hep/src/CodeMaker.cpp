...
DNNCodeMaker::DNNCodeMaker(Logger &log_, Config const &config_, InputProcessor const &inputProcessor_) : log(log_), config(config_), inputProcessor(inputProcessor_), filePy(config.GetPyFileName().c_str())
{
...
CodeMaker::CodeMaker(Logger &log_, Config const &config_, InputProcessor const &inputProcessor_,
                     FBMWrapper const &fbm_) : log(log_), config(config_), inputProcessor(inputProcessor_), fbm(fbm_),
                                               file(config.GetCPPFileName().c_str())
{
    ...
    if (not boost::filesystem::exists("BinaryDiscriminator.hpp"))
    {
        ...
        baseClassFile << "#ifndef BINARY_DISCRIMINATOR_ABSTRACT_BASE_EXTERNAL\n"
    ...
    file << "#ifdef USE_INTERNAL_BINARY_DISCRIMINATOR_ABSTRACT_BASE\n"
    ...
    file << "std::string const taskName(\"" << config.GetTaskName() << "\");\n"
    ...
    // Everything done
    log << info(2) << "The source code to apply the BNN is written in the file \"" << config.GetCPPFileName() << "\"." << eom;
}
...
void CodeMaker::WriteBNNClass()
{
    ...
    file << "class BNN: public BinaryDiscriminator\n"
         << "{"
         << "\t public:\n"
    ...
    for (unsigned i = 0; i < nTrans; ++i)
        file << "\t\tTransform" << i << " trans" << i << ";\n";

    file << "};\n\n\n";

    // Define the constructor
    file << "BNN::BNN(UInt_t netBegin_, UInt_t netEnd_)\n"
         << "{\n"
    ...
    // Now methods
    file << "void BNN::SetNetRange(UInt_t netBegin_, UInt_t netEnd_)\n"
         << "{\n"
         << "\tnetBegin = netBegin_;\n"
         << "\tnetEnd = std::min(netEnd_, UInt_t(" << nets.size() << "));\n"
         << "}\n\n\n";

    file << "Double_t BNN::operator()(Double_t const *vars) const\n"
         << "{\n"
         << "\treturn Apply(vars);\n"
         << "}\n\n\n";

    file << "Double_t BNN::operator()(Double_t var0";
    ...
    for (unsigned iVar = 0; iVar < inputProcessor.GetDim(); ++iVar)
        file << "\tvars[" << iVar << "] = var" << iVar << ";\n";

    file << "\treturn Apply(vars);\n"
         << "}\n\n\n";

    file << "Double_t BNN::Apply(Double_t const *vars) const\n"
         << "{\n"
         << "\tDouble_t transVars[" << inputProcessor.GetDim() << "];\n"
         << "\tstd::copy(vars, vars + " << inputProcessor.GetDim() << ", transVars);\n\t\n";

    for (unsigned i = 0; i < nTrans; ++i)
        file << "\ttrans" << i << "(transVars);\n";

...
void net_func(net_values *v,   /* Place to get inputs and store outputs */
...
static void bias_values(net_value *v, /* Array of unit values to set */
...
static void add_connections(net_value *s, /* Summed input for destination units to add to */
  ...
  if (omit == 0)
  {
    if (t != 0)
    {
      for (i = 0; i < ns; i++)
      {
        tv = v[i] + *t++;
        j = 0;
        // printf("add connections 1 \n");

        do
        {
          s[j] += *w++ * tv;
          j += 1;
          // printf("add connections 2 %f \n",s[i]);
        } while (j < nd);
      }
    }
    else

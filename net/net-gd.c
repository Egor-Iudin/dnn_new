     ...
     char **argv)
{
  ...
  if (*ap != 0 && strcmp(*ap, "/") != 0 && ((modulus = atoi(*ap++)) <= 0))
    usage();
       ...
       n_groups++)
  {
    ...
    if (n_groups > 0 && start_group[n_groups] < start_group[n_groups - 1])
      abort();
  }
       ...
       index++)
  {
    ...
    case Online:
    {
      ...
      {
        for (j = 0; j < np; j++)
          tgrad.param_block[j] = 0;
      }

      net_prior_prob(&params, &sigmas, 0, &gradp, arch, flgs, priors, 2);

      for (j = 0; j < np; j++)
        gradp.param_block[j] /= N_train;

      for (i = 0; i < N_train; i++)
      {
        for (j = 0; j < np; j++)
          grad.param_block[j] = gradp.param_block[j];
        ...
        {
          for (j = 0; j < np; j++)
            tgrad.param_block[j] += grad.param_block[j];
        }
      ...
      {
        for (j = 0; j < np; j++)
          ograd.param_block[j] = tgrad.param_block[j];
      }

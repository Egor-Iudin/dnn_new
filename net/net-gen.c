     ...
     char **argv)
{
  ...
  if (argc < 2 || argc > fix + 3 || fix > 2 && (max_index = atoi(argv[2])) <= 0 && strcmp(argv[2], "0") != 0 || fix + 1 < argc && (value = out_value = atof(argv[fix + 1])) <= 0 || fix + 2 < argc && (out_value = atof(argv[fix + 2])) <= 0)
  {
    fprintf(stderr,
  ...
  if (value == 1)
  {
    for (aaa = 0; aaa < w->total_params; aaa++)
    {
      w->param_block[aaa] = (float)rand() / (float)RAND_MAX;
    }
  }

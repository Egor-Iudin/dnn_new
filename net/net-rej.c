     ...
     char **argv)
{
  ...
  if (argc != 3 && argc != 4 || (max_index = atoi(argv[2])) <= 0 && strcmp(argv[2], "0") != 0 || argc > 3 && (limit = atoi(argv[3])) <= 0)
  {
    ...
    for (;;)
    {
      ...
      if (m->type == 'R')
      {
        ...
        *s->noise_cm = alpha == 0 ? m->noise.width
                                  : prior_pick_sigma(m->noise.width * sqrt(alpha / (N_train * a->N_outputs + alpha)),

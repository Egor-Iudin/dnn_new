...
int pred_app_use_index(void)
{
  ...
  for (i = 0; i < N_test; i++)
  {
    ...
    if (have_targets)
    {
      ...
      if (op_r)
      {
        for (j = 0; j < data_spec->N_targets; j++)
        {
          ...
          if (tr->take_log)
          {
            test_log_prob[i] -= log(data_inv_trans(test_targets[data_spec->N_targets * i + j], *tr));
          }
        }
      }
    }
    // printf("net_pred \n");
    // getchar();
    //  printf("test_targ_pred= %f  \n",test_targ_pred[ i*M_targets]);
    ...
    for (j = 0; j < M_targets; j++)
    {
      if (op_r)
      {
        ...
        if (tr->take_log)
        {
          if (op_n && m != 0 && m->type == 'R' && (m->noise.alpha[0] != 0 || m->noise.alpha[1] != 0 || m->noise.alpha[2] != 0))
          {
            fprintf(stderr, "Predictive mean is undefined\n");
            exit(1);
          }

          test_targ_pred[i * M_targets + j] *= exp(m->noise.width * m->noise.width / (tr->scale * tr->scale * 2));
        }
      }
    }

...
)
{
  ...
  ErrorList = (double *)chk_alloc(1, 20 * sizeof(double));
  NumbersList = (int *)chk_alloc(1, 20 * sizeof(int));
  if (need_p && ds->p == 0)
  {
  ...
  TGraph *gr1 = new TGraph(HybrydIterationsCount, NumbersList, ErrorList);
  gr1->w
      gr1->Draw("AC*");
  gr1->write();
  f.close();
        ...
        break;
          ...
          case 'T':
          {
            if (ops->op[i].in_steps == 0)
            {
              ...
              // printf("res[0] = %f \n",res[0]);
              ErrorList[HybrydIterationsCount] = res[0];

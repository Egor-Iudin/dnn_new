     ...
     char **argv)
{
  ...
  if (argc == 8 && low_width <= 0 || high_width <= 0 || n_pairs <= 0)
    usage();
  ...
  net_setup_value_pointers(v1, chk_alloc(net_setup_value_count(a), sizeof(net_value)), a);
  net_setup_value_pointers(v2, chk_alloc(net_setup_value_count(a), sizeof(net_value)), a);
  ...
  for (;;)
  {
    ...
    while (!logf.at_end && logf.header.index <= hindex && (logf.header.index < lindex || (logf.header.index - lindex) % index_mod != 0))
    {

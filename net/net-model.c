...
void net_model_prob(net_values *v,          /* Values for units in network */
    ...
    for (i = 0; i < a->N_outputs; i++)
    {
      ...
      if (1)
      {
        ...
        if (t[i] == 0)
        {
          ...
          case 0:
            ppp = 1.0 / (1 + exp(-oi));
            pprim = exp(-oi) / (1 + exp(-oi)) / (1 + exp(-oi));
            ddd = 1 / (1 - ppp) / (1 - ppp) * pprim;
            ppp = ppp / (1.0 - ppp);
            break;
          ...
          default:
            //	   			 ppp = log(1.0/(1 + exp(-oi)));
            // ppp = exp(1.0/(1 + exp(-oi)));
            // ppp = ppp*ppp;
            //	   				 ddd = exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi));
            // ddd = exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi))*(-1.0)/ppp/ppp;
            // 				 ddd = exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi))*exp(1/(1 + exp(-oi)));
            break;
          }
        }
        else
        {
          ...
          case 2:
            ...
            // default:
            // ppp = exp(1.0 - 1.0/(1 + exp(-oi)));

            // ppp = ppp*ppp;
            // ddd = 2.0*exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi))/(1 + exp(-oi)) -
            //   2.0*exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi));
            //     ddd = -exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi))*exp(1/(1 + exp(-oi)));
            //	       ddd = -exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi));
            // ddd = exp(-oi)/(1 + exp(-oi))/(1 + exp(-oi))*(-1.0)/(1.0 - ppp)/(1.0 - ppp);
            // break;
          }
        }
        // srand (time(NULL));
        // ddd = ddd*((float)rand()/(float)RAND_MAX);
        if (pr)
          *pr -= ppp;
        if (dp)
          dp->o[i] = ddd;
...
void net_model_max_second(net_value *msd,         /* Place to store maximum second derivatives */
                          net_arch *a,            /* Network architecture */
                          model_specification *m, /* Data model */
                          model_survival *sv,     /* Type of hazard function for survival model, or null*/
                          net_sigmas *s           /* Hyperparameters, including noise sigmas */
...
void net_model_guess(net_values *v,          /* Values for units in network */

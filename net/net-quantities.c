...
void net_initialize(log_gobbled *logg)
{
  ...
  if (sigmas.sigma_block == 0 || params.param_block == 0 || logg->index['S'] != logg->index['W'])
  {
...
void net_available(quantities_described qd,
                   log_gobbled *logg)
{
  ...
  for (v = 0; v < Max_quantities; v++)
  {
    ...
    {
      if (strchr("ixoygtzlbav", letter) != 0 && !have_train_data || strchr("IXOYGTZLBAV", letter) != 0 && !have_test_data || strchr("ZBA", letter) != 0 && !have_test_targets)
      {
        qd[v].available = -1;
        continue;
      }
      ...
      else if (letter == 'h')
      {
        ...
        if (qd[v].available == 1 && qd[v].low != -1)
        {
          if (qd[v].high == -1)
            qd[v].high = n - 2;
          if (n <= 1 || qd[v].high > n - 2 || qd[v].low > n - 2)
            qd[v].available = -1;
        }
      }
      else if (letter == 'w')
      {
        qd[v].available = qd[v].low != -1 &&
                                  net_setup_param_group(arch, flgs, mod, &o, &n, &s)
                              ? 1
                              : -1;
        if (qd[v].available == 1)
        {
          if (qd[v].high == -1)
            qd[v].high = n - 1;
          if (qd[v].high > n - 1 || qd[v].low > n - 1)
            qd[v].available = -1;
        }
      }
      else if (letter == 'n' || letter == 'N')
      {
        qd[v].available = (model_type == 'R' || model_type == 'C') && mod == -1 ? 1 : -1;
      }
      ...
      else if (letter == 'W')
      {
        ...
        if (qd[v].available == 1 && qd[v].low != -1)
        {
          if (qd[v].high == -1)
            qd[v].high = s - 1;
          if (s == 0 || qd[v].high > s - 1 || qd[v].low > s - 1)
            qd[v].available = -1;
        }
      }
      else if (letter == 'M')
...
void net_evaluate(quantities_described qd,
                  quantities_held *qh,
                  log_gobbled *logg)
{
  char model_type = model ? model->type : 0;
  ...
  if (logg->data['S'] == 0 || logg->index['S'] != logg->last_index || logg->data['W'] == 0 || logg->index['W'] != logg->last_index)
  {
  ...
  for (v = 0; v < Max_quantities; v++)
  {
    ...
    if (letter && !qh->updated[v] && (strchr("iItT", letter) == 0 || mod != -1))
    {
      ...
      if (!ev_train && (strchr("uoyglbacv", letter) != 0 || letter == 'n' && model_type == 'C'))
      {
        if (!have_train_data)
          abort();
        for (i = 0; i < N_train; i++)
      ...
      if (!ev_test && (strchr("OYGLBACV", letter) != 0 || letter == 'N' && model_type == 'C'))
      {
        if (!have_test_data)
          abort();
        for (i = 0; i < N_test; i++)
      ...
      case 'h':
      {
        ...
        if (!net_setup_hyper_group(arch, flgs, mod, &o, &n, &s))
          abort();
      ...
      case 'w':
      {
        ...
        if (!net_setup_param_group(arch, flgs, mod, &o, &n, &s))
          abort();

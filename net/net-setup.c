...
int net_setup_sigma_count(net_arch *a,           /* Network architecture */
...
int net_setup_param_count(net_arch *a,    /* Network architecture */
                          net_flags *flgs /* Network flags, null if none */
)
{
  int count;
  ...
  for (l = 0; l < a->N_layers; l++)
  {
    ...
    if (a->has_ih[l])
    {
      count += not_omitted(flgs ? flgs->omit : 0, a->N_inputs, 1 << (l + 1)) * a->N_hidden[l];
    }
  ...
  if (a->has_io)
  {
    count += not_omitted(flgs ? flgs->omit : 0, a->N_inputs, 1) * a->N_outputs;
  }
...
int net_setup_value_count(net_arch *a /* Network architecture */
)
{
  int count;
  int l;
...
void net_setup_sigma_pointers(net_sigmas *s,         /* Structure to set up pointers in */
...
void net_setup_param_pointers(net_params *w,  /* Structure to set up pointers in */
                              net_arch *a,    /* Network architecture */
                              net_flags *flgs /* Network flags, null if none */
)
{
  ...
  for (l = 0; l < a->N_layers; l++)
  {
    ...
    {
      w->ih[l] = b;
      b += not_omitted(flgs ? flgs->omit : 0, a->N_inputs, 1 << (l + 1)) * a->N_hidden[l];
    }
...
void net_setup_value_pointers(net_values *v, /* Structure to set up pointers in */
...
int net_setup_hyper_group(net_arch *a,     /* Network architecture */
                          net_flags *flgs, /* Network flags, null if none */
...
int net_setup_param_group(net_arch *a,     /* Network architecture */
                          net_flags *flgs, /* Network flags, null if none */
                          int grp,         /* Index of group */
                          int *offset,     /* Set to offset of group within block */
                          int *number,     /* Set to number of items in group */

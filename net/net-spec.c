     ...
     char **argv)
{
  ...
  if (argc == 2)
  {
    ...
    for (l = 0; l < a->N_layers; l++)
    {
      ...
      if (flgs == 0 || flgs->layer_type[l] == Tanh_type)
        ...
        printf("  UNKNOWN TYPE!");
      if (flgs != 0 && l < 7 && list_flags(flgs->omit, a->N_inputs, 1 << (l + 1), ps) > 0)
      {
        printf("  omit%s", ps);
      }
      printf("\n");
    ...
    if (flgs != 0 && l < 7 && list_flags(flgs->omit, a->N_inputs, 1, ps) > 0)
    {
      printf("  omit%s", ps);
    }
    printf("\n");
  ...
  while (*ap != 0 && strcmp(*ap, "/") != 0)
  {
    ...
    while ((*ap)[0] >= 'a' && (*ap)[0] <= 'z')
    {
      if ((*ap)[0] == 'o' && (*ap)[1] == 'm' && (*ap)[2] == 'i' && (*ap)[3] == 't' && (*ap)[4] == ':')
      {
        if (omit)
          usage();
        omit = 1;
  ...
  if (*++ap == 0 || (a->has_ti = strcmp(*ap, "-") != 0) && !prior_parse(&p->ti, *ap))
    usage();
  ...
  if (a->N_layers > 0)
  {
    for (l = 0; l < a->N_layers; l++)
    {
      if (l > 0)
      {
        if (*++ap == 0 || (a->has_hh[l - 1] = strcmp(*ap, "-") != 0) && !prior_parse(&p->hh[l - 1], *ap))
          usage();
      }

      if (*++ap == 0 || (a->has_ih[l] = strcmp(*ap, "-") != 0) && !prior_parse(&p->ih[l], *ap))
        usage();

      if (*++ap == 0 || (a->has_bh[l] = strcmp(*ap, "-") != 0) && !prior_parse(&p->bh[l], *ap))
        usage();

      if (*++ap == 0 || (a->has_th[l] = strcmp(*ap, "-") != 0) && !prior_parse(&p->th[l], *ap))
        usage();
    }

    for (l = a->N_layers - 1; l >= 0; l--)
    {
      if (*(ap + 1) == 0 || strcmp(*(ap + 1), "/") == 0 || *(ap + 2) == 0 || strcmp(*(ap + 2), "/") == 0 || *(ap + 3) == 0 || strcmp(*(ap + 3), "/") == 0)
      {
        a->has_ho[l] = 0;
      }
      else
      {
        if (*++ap == 0 || (a->has_ho[l] = strcmp(*ap, "-") != 0) && !prior_parse(&p->ho[l], *ap))
          usage();
      }
    }
  }

  if (*++ap == 0 || (a->has_io = strcmp(*ap, "-") != 0) && !prior_parse(&p->io, *ap))
    usage();

  if (*++ap == 0 || (a->has_bo = strcmp(*ap, "-") != 0) && !prior_parse(&p->bo, *ap))
    usage();

  if (*++ap != 0 && strcmp(*ap, "/") == 0 && *(ap + 1) != 0 && strchr("+-.0123456789", **(ap + 1)) != 0)
  {
    ap += 1;
    ...
    for (l = 0; l < a->N_layers; l++)
    {
      ...
      if (a->has_ah[l] && (p->ah[l] = atof(*ap)) <= 0)
        usage();
      ap += 1;
    }
  ...
  if (p->ti.scale || p->ti.alpha[2] != 0 || p->bo.scale || p->bo.alpha[2] != 0)
  {
  ...
  for (l = 0; l < a->N_layers; l++)
  {
    if (p->bh[l].scale || p->bh[l].alpha[2] != 0 || p->th[l].scale || p->th[l].alpha[2] != 0)
    {
      fprintf(stderr, "Illegal prior for biases or offsets\n");
      exit(1);
    }

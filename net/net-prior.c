...
void net_prior_generate(net_params *w,          /* Arrays to store weights, biases, and offsets in */
...
static void pick_unit_params(net_param *wt,    /* Array to store parameters */
...
static void pick_weights(net_param *wt,    /* Array to store weights */
                         net_sigma *sd_cm, /* Place to store common sigma */
...
)
{
  ...
  *sd_cm = centre ? (value == 0 || pr.alpha[0] == 0 ? width : value)
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (value == 1)
    {
      sd[i] = (float)rand() / (float)RAND_MAX;
    }
    printf("sd[i]= %f \n", sd[i]);
    for (j = 0; j < nd; j++)
    {
...
void net_prior_prob(net_params *w,   /* Weights, biases, offsets, etc. */
...
)
{
  ...
  if (a->has_ti)
    compute_prior(w->ti, a->N_inputs, lp, dp ? dp->ti : 0,
  ...
  for (l = 0; l < a->N_layers; l++)
  {
    ...
    if (a->has_th[l])
      compute_prior(w->th[l], a->N_hidden[l], lp,
...
static void compute_prior(net_param *wt,   /* Set of parameters */
...
void net_prior_max_second(net_params *d,   /* Place to store maximum second derivatives */
...
static void max_second(net_param *d, /* Place to store maximum second derivatives */

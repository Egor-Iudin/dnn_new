     ...
     char **argv)
{
  ...
  if (sscanf(argv[2], "%d%c", &ix1, &junk) != 1 || sscanf(argv[4], "%d%c", &ix2, &junk) != 1)
  {
    usage();
  }
  ...
  for (i = 1; i < 128; i++)
  {
    if (!strchr(ignore, i))
    {
      ...
      {
        if (logg1.index[i] != ix1 || logg2.index[i] != ix2 || logg1.actual_size[i] != logg2.actual_size[i] || memcmp(logg1.data[i], logg2.data[i], logg1.actual_size[i]) != 0)
        {
          exit(1);
        }
      }

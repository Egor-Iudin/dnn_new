...
)
{
  ...
  x = init;
  f = logp(x, &df, extra); /* find point to the left of mode */
  max = f;
  ...
  x = init;
  f = logp(x, &df, extra); /* find point to the right of mode */
  while (df > -TINY || f > max - MIN_DROP)
  {
    x += scale + (x - init);
    f = logp(x, &df, extra);
  ...
  for (i = 0;; i++)
  { /* repeat until a point is accepted */
        ...
        (!(cur->nxt) || fabs(df - cur->nxt->a) > TINY))
    {
      ...
      if (x < cur->x)
        cur = cur->prv; /* now, insert *after* cur */
      prv = cur;
      cur = &seg[no_seg++];
      cur->prv = prv;
      if (prv)
      ...
      if (prv)
        prv->xmax = (cur->b - prv->b) / (prv->a - cur->a);
      if (nxt)
        cur->xmax = (nxt->b - cur->b) / (cur->a - nxt->a);
    }
  }
}

     ...
     char **argv)
{
  ...
  if (argc == 8 && (op_z = strchr(argv[1], 'z')))
  {
    ROOT_output = 1;
    printf("Start Root initialization \n");
    ...
    for (i = 0; !(argv[7][i] == ':' && argv[7][i + 1] == '/') && argv[7][i] != '\0'; i++)
      ROOT_file_name[i] = argv[7][i];

    ROOT_file_name[i] = '\0';

    if (argv[7][i] == '\0') // ':/' not found
    {
      fprintf(stderr, "ROOT file name must be of the form \"filename.root:/[dir1/[dir2/[...]]]tree\"\n");
      exit(1);
    }

    char *last_slash = strrchr(argv[7] + i + 2, '/');
    ...
    if (last_slash) // last_slash == NULL if no in-file directory is specified
    {
      ...
      *last_slash = '\0'; // argv[6] is never user again, we can change it
      strcpy(ROOT_dir_name, argv[7] + i + 2);
      strcpy(ROOT_tree_name, last_slash + 1);
    }
    else
    {
      *ROOT_dir_name = '\0';
      strcpy(ROOT_tree_name, argv[7] + i + 2);
    }
  ...
  if (strlen(options) != op_i + op_t + op_r + op_p + op_m + op_n + op_d + op_D + op_E + op_b + op_a + op_q + op_Q + op_l + op_N)
  {
    usage();
  }
  ...
  if (op_a && (op_t || op_i || op_q || op_Q) || op_r && (m != 0 && (m->type == 'B' || m->type == 'C')) || op_l && (m == 0 || m->type != 'R') || op_m && (m == 0 || m->type == 'R' || m->type == 'V') || (op_d || op_D || op_q || op_Q) && (m != 0 && m->type == 'B' || m != 0 && m->type == 'C'))
  {
    ...
    for (;;)
    {
      ...
      if (**ap == '@') /* Range gives indexes via cpu times */
      {
        ...
        while (!logfile.at_end && lindex == -1)
        {
          ...
          if (logg.last_index >= 0 && logg.data['i'] != 0 && logg.index['i'] == logg.last_index)
          {
            it = logg.data['i'];
      ...
      for (;;)
      {
        ...
        while (!logfile.at_end && logfile.header.index <= hindex && (logfile.header.index < lindex || (mod_no > 0 ? logfile.header.index % mod_no != 0 : logfile.header.index < (int)(target_index + 0.5))))
        {
        ...
        if (mod_no < 0)
          target_index += (double)(hindex - lindex) / (-mod_no - 1);
      }
  ...
  if (ROOT_output)
  {
    ...
    if (op_t)
      CTTree_Branch(ROOT_tree, "t", &ROOT_br_t, "t/D");
  }
  ...
  for (i = 0; i < N_test; i++)
  {
    if (put_in_blanks)
    {
      if (i > 0 && test_inputs[data_spec->N_inputs * i + 0] != test_inputs[data_spec->N_inputs * (i - 1) + 0])
      {
        printf("\n");
      }
    }
    ...
    if (op_p)
    {
      if (have_targets)
      {
        ...
        if (!op_a)
          printf(op_b ? " %.8e" : " %9.3f", log_prob[i]);
      }
      if (op_E)
      {
        double pa, pb, E_lp;
    ...
    if (op_m)
    {
      ...
      {
        printf(" ");
        if (data_spec->N_targets < 2)
          printf("   ");
        for (j = 0; j < data_spec->N_targets; j++)
        {
          printf(" %2.0f", guess[j]);
        }
        if (data_spec->N_targets < 3)
          printf("  ");
        if (have_targets)
        {
          printf(" ");
    ...
    if (op_n)
    {
      ...
      for (j = 0; j < M_targets; j++)
      {
        ...
        if (op_E)
        {
          E_error[j] = guess[j] * (1 - guess[j]) * (1 - guess[j]) + (1 - guess[j]) * guess[j] * guess[j];
  ...
  if (have_targets && guessing && !op_b)
  {
    ...
    if (op_n && !op_R)
    {
      ...
      if (m != 0 && m->type == 'C')
      {
        ...
        printf(" %8.5f", a);
        FILE *ErrFile;
        ErrFile = fopen("QuardError.txt", "w+");
        fprintf(ErrFile, " %8.5f  ", a);
        fclose(ErrFile);
        if (N_test > 1)
        {
      ...
      else
      {
        for (j = 0; j < data_spec->N_targets; j++)
        {
          ...
          printf(" %8.5f", a);
          FILE *ErrFile;
          ErrFile = fopen("QuardError.txt", "a");
          fprintf(ErrFile, " %8.5f  ", a);
          fclose(ErrFile);
          if (N_test > 1)
          {
...
{
  return *(float *)a > *(float *)b ? +1 : *(float *)a < *(float *)b ? -1
                                                                    : 0;
}

static float find_median(float *data, /* Array of data values */

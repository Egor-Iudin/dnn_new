...
void log_file_create(log_file *logf /* Log file state structure */
)
{
...
void log_file_open(log_file *logf,  /* Log file state structure */
...
void log_file_close(log_file *logf /* Log file state structure */
)
{
...
void log_file_first(log_file *logf /* Log file state structure */
)
{
...
void log_file_last(log_file *logf /* Log file state structure */
)
{
  log_header trailer;
...
void log_file_forward(log_file *logf /* Log file state structure */
)
{
  log_header trailer;
...
void log_file_backward(log_file *logf /* Log file state structure */
)
{
  log_header trailer;
...
void log_file_read(log_file *logf, /* Log file state structure */
...
void log_file_append(log_file *logf, /* Log file state structure */
...
)
{
  ...
  if (!logf->last_index_known)
  {
    ...
    else
    {
      ...
      if (fread(&trailer, 1, sizeof trailer, logf->file_struct) != sizeof trailer || fseek(logf->file_struct, 0, 2) != 0)
      {
        fprintf(stderr,
  ...
  if (log_append_compare)
  {
    ...
    {
      if (log_append_compare->actual_size[i] != logf->header.size || memcmp(log_append_compare->data[i], data, logf->header.size) != 0)
      {
...
static void read_header(log_file *logf, /* Log file state structure */
...
static void check_header_trailer(log_header *h,
                                 log_header *t)
{
...
void log_gobble_init(log_gobbled *logg, /* Records set of records gobbled up */
...
void log_gobble(log_file *logf,   /* Log file state structure */
...
int log_gobble_last(log_file *logf,   /* Log file state structure */
                    log_gobbled *logg /* Structure for holding gobbled data */
)
{
  int index;

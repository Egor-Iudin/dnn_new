     ...
     char **argv)
{
  ...
  if (strlen(options) != op_m + op_s + op_v + op_a + op_c + op_b + op_e)
    usage();
  ...
  {
    max_lag = atoi(argv[2]);
    if (max_lag <= 0 && strcmp(argv[2], "0") != 0)
      usage();
  }
  ...
  if (op_v || op_s)
  {
    ...
    if (op_s)
      printf("Standard deviation: %.6f  ", sqrt(variance));
    printf("\n");
  }
  ...
  if ((op_a || op_c) && variance > 0)
  {
    ...
    for (l = 1; l <= ml; l++)
    {
      ...
      if (op_a)
        printf("%9.6f ", (cvs[l] / cvn[l]) / variance);

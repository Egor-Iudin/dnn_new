...
char *formula_def(char *def, /* The definition */
...
double formula(char *form0,    /* The formula, as a character string */
...
static double expr(double gr[26][11])
{
  double gr2[26][11];
  double v, v2;
  int k;
...
static double term(double gr[26][11])
{
  double gr2[26][11];
  double v, v2;
  char op;
  ...
  while (*nch != 0 && *nch != '+' && *nch != '-' && *nch != ')' && *nch != ']' && *nch != '}' && *nch != ',')
  {
    op = *nch;
    if (op == '*' || op == '/')
      next();
    ...
    if (op == '/')
    {
      if (gradvars && (v != 0 || grnz))
      {
        for (k = 0; k < n_gv; k++)
        {
          gr[c_gv[k]][i_gv[k]] = gr[c_gv[k]][i_gv[k]] / v2 - v * gr2[c_gv[k]][i_gv[k]] / (v2 * v2);
        }
      }
      if (eval && v != 0)
      {
    ...
    else
    {
      if (gradvars && (v != 0 || grnz))
      {
        for (k = 0; k < n_gv; k++)
        {
          gr[c_gv[k]][i_gv[k]] = gr[c_gv[k]][i_gv[k]] * v2 + v * gr2[c_gv[k]][i_gv[k]];
        }
      }
      if (eval && v != 0)
      {
...
static double factor(double gr[26][11])
{
  double gr2[26][11];
  double v, w;
  int i, k;
...
static double prefactor(double gr[26][11])
{
  double v;
  int k;
  ...
  case 'C':
  {
    ...
    else if (nch[0] == 'C' && nch[1] == 'o' && nch[2] == 's' && nch[3] == 'h' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
  ...
  case 'D':
  {
    if (nch[0] == 'D' && nch[1] == 'e' && nch[2] == 'l' && nch[3] == 't' && nch[4] == 'a' && !LOWER(nch[5]))
    {
      nch += 4;
      next();
      v = prefactor(gr);
  ...
  case 'E':
  {
    if (nch[0] == 'E' && nch[1] == 'x' && nch[2] == 'p' && nch[3] == 'G' && nch[4] == 'a' && nch[5] == 'm' && nch[6] == 'm' && nch[7] == 'a' && nch[8] == '2' && !DIGIT(nch[9]))
    {
      nch += 8;
      next();
      v = expgamma2(0, v, gr);
    ...
    else if (nch[0] == 'E' && nch[1] == 'x' && nch[2] == 'p' && nch[3] == 'G' && nch[4] == 'a' && nch[5] == 'm' && nch[6] == 'm' && nch[7] == 'a' && !LOWER(nch[8]))
    {
      nch += 7;
      next();
      v = expgamma(0, v, gr);
  ...
  case 'F':
  {
    if (nch[0] == 'F' && nch[1] == 'r' && nch[2] == 'a' && nch[3] == 'c' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
      if (eval)
        v = v >= 0 ? fmod(v, 1.0) : 1 - fmod(-v, 1.0);
    }
  ...
  case 'G':
  {
    if (nch[0] == 'G' && nch[1] == 'a' && nch[2] == 'u' && nch[3] == 's' && nch[4] == 's' && nch[5] == 'i' && nch[6] == 'a' && nch[7] == 'n' && !LOWER(nch[8]))
    {
      nch += 7;
      next();
      v = gaussian(0, v, gr);
  ...
  case 'L':
  {
    if (nch[0] == 'L' && nch[1] == 'G' && nch[2] == 'a' && nch[3] == 'm' && nch[4] == 'm' && nch[5] == 'a' && !LOWER(nch[6]))
    {
      nch += 5;
      next();
      v = prefactor(gr);
    ...
    else if (nch[0] == 'L' && nch[1] == 'o' && nch[2] == 'g' && nch[3] == 'S' && nch[4] == 'u' && nch[5] == 'm' && nch[6] == 'E' && nch[7] == 'x' && nch[8] == 'p' && !LOWER(nch[9]))
    {
      char close;
      ...
      while (*nch == ',')
      {
        ...
        if (eval)
          sum = v > v2 ? v + log(1 + exp(v2 - v)) : v2 + log(1 + exp(v - v2));
        ...
        if (gradvars)
        {
          ...
          for (k = 0; k < n_gv; k++)
          {
            gr[c_gv[k]][i_gv[k]] = f * gr[c_gv[k]][i_gv[k]] + f2 * gr2[c_gv[k]][i_gv[k]];
          }
        }
  ...
  case 'N':
  {
    if (nch[0] == 'N' && nch[1] == 'o' && nch[2] == 'r' && nch[3] == 'm' && nch[4] == 'a' && nch[5] == 'l' && !LOWER(nch[6]))
    {
      nch += 5;
      next();
      v = gaussian(0, v, gr);
  ...
  case 'S':
  {
    if (nch[0] == 'S' && nch[1] == 'i' && nch[2] == 'g' && nch[3] == 'n' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
    ...
    else if (nch[0] == 'S' && nch[1] == 'i' && nch[2] == 'n' && nch[3] == 'h' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
    ...
    else if (nch[0] == 'S' && nch[1] == 'q' && nch[2] == 'r' && nch[3] == 't' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
  ...
  case 'T':
  {
    ...
    else if (nch[0] == 'T' && nch[1] == 'a' && nch[2] == 'n' && nch[3] == 'h' && !LOWER(nch[4]))
    {
      nch += 3;
      next();
      v = prefactor(gr);
    ...
    else if (nch[0] == 'T' && nch[1] == 'h' && nch[2] == 'e' && nch[3] == 't' && nch[4] == 'a' && !LOWER(nch[5]))
    {
      nch += 4;
      next();
      v = prefactor(gr);
  ...
  case 'z':
  {
    ...
    else if (nch[0] == 'G' && nch[1] == 'a' && nch[2] == 'u' && nch[3] == 's' && nch[4] == 's' && nch[5] == 'i' && nch[6] == 'a' && nch[7] == 'n' && !LOWER(nch[8]))
    {
      nch += 7;
      next();
      v = gaussian(1, v, gr);
    ...
    else if (nch[0] == 'E' && nch[1] == 'x' && nch[2] == 'p' && nch[3] == 'G' && nch[4] == 'a' && nch[5] == 'm' && nch[6] == 'm' && nch[7] == 'a' && nch[8] == '2' && !DIGIT(nch[9]))
    {
      nch += 8;
      next();
      v = expgamma2(1, v, gr);
...
static double var(double gr[26][11])
{
  int c, i, k;
...
static double number(double gr[26][11])
{
  char n[101];
  int k, dot;
  double v;
...
static double gaussian(int tilde,        /* Does this call use the tilde syntax? */
...
)
{
  ...
  if (gradvars)
  {
    ...
    {
      gr[c_gv[k]][i_gv[k]] =
          (v - v2) / v3 * (gr[c_gv[k]][i_gv[k]] - gr2[c_gv[k]][i_gv[k]]) + gr3[c_gv[k]][i_gv[k]] * (1 / (2 * v3) - (v - v2) * (v - v2) / (2 * v3 * v3));
    }
  }

  if (eval)
    v = log(2 * M_PI * v3) / 2 + (v - v2) * (v - v2) / (2 * v3);

  return v;
}

static double expgamma2(int tilde,        /* Does this call use the tilde syntax? */
                        double v,         /* Set to value if tilde syntax used */
                        double gr[26][11] /* Gradient, already set if tilde syntax used */
)
{
    ...
    for (k = 0; k < n_gv; k++)
    {
      gr[c_gv[k]][i_gv[k]] =
          ...
          (gr2[c_gv[k]][i_gv[k]] == 0 ? 0
                                      : (-0.5 * log((v2 / 2) / v3) - 0.5 + digamma(v2 / 2) / 2 + exp(v) / (2 * v3) - v / 2) * gr2[c_gv[k]][i_gv[k]]) +
          ((v2 / 2) / v3 - exp(v) * (v2 / 2) / (v3 * v3)) * gr3[c_gv[k]][i_gv[k]];
    }
  }

  if (eval)
    v = -(v2 / 2) * log((v2 / 2) / v3) + lgamma(v2 / 2) + exp(v) * (v2 / 2) / v3 - (v2 / 2) * v;

  return v;
}

static double expgamma(int tilde,        /* Does this call use the tilde syntax? */
                       double v,         /* Set to value if tilde syntax used */
                       double gr[26][11] /* Gradient, already set if tilde syntax used */
)
{
  ...
  if (eval)
    v = -v2 * log(v3) + lgamma(v2) + exp(v) * v3 - v2 * v;
...
void formula_sample(char *form0,    /* Distribution formula */
...
)
{
  ...
  while (*nch != 0)
  {
    ...
    if (nch[0] == 'N' && nch[1] == 'o' && nch[2] == 'r' && nch[3] == 'm' && nch[4] == 'a' && nch[5] == 'l' && !LOWER(nch[6]) || nch[0] == 'G' && nch[1] == 'a' && nch[2] == 'u' && nch[3] == 's' && nch[4] == 's' && nch[5] == 'i' && nch[6] == 'a' && nch[7] == 'n' && !LOWER(nch[8]))
    {
      nch += nch[0] == 'N' ? 5 : 7;
      next();
...
static double ext_func(int tilde, /* Is this a reference via tilde notation? */
                       double v   /* Set to value if tilde syntax used */
)
{
  ...
  if (eval)
  {
    ...
    if (fwrite(&ext_head, sizeof ext_head, 1, ext_funcs[ei].to) != 1 || fwrite(ext_args, sizeof(double), ext_head.n_args, ext_funcs[ei].to) != ext_head.n_args || fflush(ext_funcs[ei].to) == EOF)
    {
